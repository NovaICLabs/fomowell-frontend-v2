...
import {
	useBtcCalculateSwapRunes,
	useBtcCalculateSwapSats,
...
export default function SwapTrade({ initialTab }: { initialTab?: TradeTab }) {
	...
	const { mutateAsync: buy, isPending: isBuying } = useBtcSwapRunes();
	const { mutateAsync: sell, isPending: isSelling } = useBtcSwapSats();
	...
	const handleConfirm = useCallback(async () => {
		...
		try {
			switch (activeTab) {
				case "Buy":
					if (debouncedBuyAmountBigInt && calculatedBuyReceived?.nonce) {
						const result = await buy({
							sats: debouncedBuyAmountBigInt,
							id: BigInt(id),
							runes_min: minTokenReceived,
							nonce: calculatedBuyReceived?.nonce,
						});
					...
					break;
				case "Sell":
					if (debouncedSellAmountBigInt && calculatedSellReceived?.nonce) {
						const result = await sell({
							runes: debouncedSellAmountBigInt,
							id: BigInt(id),
							sats_min: minTokenReceived,
							nonce: calculatedSellReceived?.nonce,
						});
						console.debug("🚀 ~ handleConfirm ~ result:", result);
						showToast(
							"success",
		...
		minTokenReceived,
		slippage,
		calculatedBuyReceived?.nonce,
		calculatedSellReceived?.nonce,
		activeTab,
		debouncedBuyAmountBigInt,
		debouncedSellAmountBigInt,
		buy,
