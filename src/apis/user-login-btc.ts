...
export const loginOrRegisterByPrincipal = async (
	...
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/auth/login`, {
		method: "POST",
		headers: {
...
export const refreshToken = async (token: string) => {
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/auth/refresh`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
...
export const checkLogin = async (token: string): Promise<boolean> => {
	...
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/auth/verfiy?token=${token}`);
...
export const createNewUser = async () => {
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/users/create`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
		},
...
export const getUserInfoByToken = async (user_token: string) => {
	...
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/users/user`, {
		method: "GET",
		headers: {
			"Content-Type": "application/json",
...
export const updateUserInfo = async (
	...
	const response = await request<{
		...
		statusCode: number;
		message: string;
	}>(`${getIndexerBtcBaseUrl()}/api/v1/users/update`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
			authorization: `Bearer ${user_token}`,
